...
POI = function(anchor, title, options) {
    ...
    this.link  = $('<a>').text(this.title)
                         ...
                         .appendTo(this.node);
    this.node.css('right',
                  '-' + (this.node.outerWidth() - this.peak) + 'px');
    this.refresh();
...
POI.whereami = function() {
    ...
    jQuery.each(POI.all,
                function() {
                    if (position < this.anchor.offset().top)
                        return false;
                    last = this;
                    //return true; // ???
                });
...
$(document).ready(function() {
    $('.section > h1 > a.headerlink, .section > h2 > a.headerlink, .section > h3 > a.headerlink').each(function(index) {
        var lnk = $(this);
