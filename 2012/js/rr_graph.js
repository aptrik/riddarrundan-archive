        ...
        if (item) {
            if (previousPoint != item.datapoint) {
                ...
                showTooltip(item.pageX, item.pageY,
                            "På hål " + item.datapoint[0] + " har "
                            + item.series.label + " i snitt "
                            + item.datapoint[1] + " kast.");
            }
        }
        else {
            $("#tooltip").remove();
...
function rr_summary_ctp_graph(id, data, colors) {
    ...
    var ymax = 5;
    // TODO: find ymax for each hole in each series.
    for (var x in data) {
        var d = x['data'];
        var sum = 0;
        for (var pair in d) {
    ...
    if (data.length > 1) { // TODO: remove when find ymax works properly.
    ...
    placeholder.bind("plothover", function (event, pos, item) {
        ...
        if (item) {
            if (previousPoint != item.datapoint) {
                ...
                showTooltip(item.pageX, item.pageY,
                            "På hål " + item.datapoint[0] + " har "
                            + item.series.label + " gjort "
                            + (item.datapoint[1] - item.datapoint[2]) + " CTP.");
            }
        }
        else {
            $("#tooltip").remove();
